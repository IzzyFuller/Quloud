...
class ProofRequestHandler:
    ...
    Decrypts the stored blob using its per-document key and computes proof.
    """
    ...
    def handle(self, request: ProofRequestMessage) -> None:
        ...
        Looks up the per-document key, decrypts the stored blob,
        then computes proof on the plaintext data.
